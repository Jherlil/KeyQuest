/*
  ------------------------------------------------------------------------------
  KeyQuest V1.3 AVX512 by Benjade https://github.com/Benjade
...
/* ── SHA-256: une passe 16-wide AVX512 ───────────────── */
sha256_avx512_16B(
    ...
    shaOut[0].data(),  shaOut[1].data(),  shaOut[2].data(),  shaOut[3].data(),
    shaOut[4].data(),  shaOut[5].data(),  shaOut[6].data(),  shaOut[7].data(),
    shaOut[8].data(),  shaOut[9].data(),  shaOut[10].data(), shaOut[11].data(),
    shaOut[12].data(), shaOut[13].data(), shaOut[14].data(), shaOut[15].data()
);

/* ── Préparation des blocs RIPEMD ──────────────── */
for(int i=0; i<bCount; ++i)
...
for(int i=0; i<HASH_BATCH_SIZE; ++i){
    ...
    outPtr[i] = ripemdOut[i].data();
}

/* ── RIPEMD-160: une passe 16-wide AVX512 ───────────── */
ripemd160avx512::ripemd160avx512_32(
    inPtr[0],   inPtr[1],   inPtr[2],   inPtr[3],
    inPtr[4],   inPtr[5],   inPtr[6],   inPtr[7],
    inPtr[8],   inPtr[9],   inPtr[10],  inPtr[11],
    inPtr[12],  inPtr[13],  inPtr[14],  inPtr[15],
    ripemdOut[0].data(),  ripemdOut[1].data(),  ripemdOut[2].data(),  ripemdOut[3].data(),
    ripemdOut[4].data(),  ripemdOut[5].data(),  ripemdOut[6].data(),  ripemdOut[7].data(),
    ripemdOut[8].data(),  ripemdOut[9].data(),  ripemdOut[10].data(), ripemdOut[11].data(),
    ripemdOut[12].data(), ripemdOut[13].data(), ripemdOut[14].data(), ripemdOut[15].data()
);

/* ── Copie vers la sortie finale ───────────────── */
for(int i=0; i<bCount; ++i){
    int idx = b * HASH_BATCH_SIZE + i;
...
// -----------------------------------------------------------------------------
// Match recording function
static void saveMatchInfo(const std::string &priv,
                          ...
                          double pct)
{
    std::ofstream f("match_report.txt", std::ios::app);
    if (f) {
        f << "Private Key: "   << priv << '\n'
          << "Public Key: "    << pub  << '\n'
          << "WIF: "          << wif  << '\n'
          ...
          << "Speed: "        << std::fixed << std::setprecision(2) << speed << " Mkeys/s\n"
...
// After filling privBin and building batchKey
Int baseKey; baseKey.Set32Bytes(privBin);

// If debug mode, show all generated keys :
if (DEBUG_MODE) {
    std::lock_guard<std::mutex> lk(debugMutex);
    std::cout << "\033[H\033[J";
    std::cout << "=== DEBUG: clés privées (batch " << g_pointsBatchSize << ") ===\n";

    for (int off = 0; off < 2*g_pointsBatchSize; ++off) {
        Int tmp = baseKey;
        if (off < g_pointsBatchSize) {
            Int o; o.SetInt32(off);
            tmp.Add(&o);
        } else {
            Int o; o.SetInt32(off - g_pointsBatchSize);
            tmp.Sub(&o);
        }
        std::string hex = intToHex(tmp);
        if (hex.size() < 64)
            hex = std::string(64-hex.size(),'0') + hex;
        std::cout << "[" << std::setw(3) << off << "] " << hex << "\n";
    }
    std::cout << "===========================\n";
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
			    ...
			    for (int k = 0; k < localBatch; ++k) {
    			    // compares the 20 bytes of the hash160
    			    if (std::memcmp(hashOut[k].data(), targetHash.data(), 20) == 0) {
        		    #pragma omp critical
        		    if (!g_found.load()) {
                                    g_found.store(true);
